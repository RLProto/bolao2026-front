...
class RankingBonusItem(BaseModel):
    user_id: int
    user_name: str
    total_points: int
    base_points: int
    champion_alive: bool = False
    exact_scores: int = 0
    correct_results: int = 0
    winner_goals: int = 0
    loser_goals: int = 0
...
def _compute_alive_teams(db: Session) -> set:
    """Retorna set de team_ids ainda vivos no torneio (fase eliminatória)."""
    rows = db.execute(text("""
        SELECT m.id, m.home_team_id, m.away_team_id,
               m.home_score, m.away_score, m.kickoff_at_utc,
               ht.name as home_name, at2.name as away_name
        FROM matches m
        JOIN teams ht  ON ht.id  = m.home_team_id
        JOIN teams at2 ON at2.id = m.away_team_id
        WHERE m.stage NOT ILIKE '%grupo%'
          AND ht.name  != 'A definir'
          AND at2.name != 'A definir'
    ...
    for r in rows:
        ...
        if r.home_score > r.away_score:
            eliminated.add(r.away_team_id)
        elif r.away_score > r.home_score:
            eliminated.add(r.home_team_id)
        # empate → penaltis: determina quem avançou olhando fases posteriores

    # Para empates, verifica qual time aparece num jogo posterior
    for r in rows:
        if r.home_score is None or r.away_score is None or r.home_score != r.away_score:
            continue
        home_in_later = any(
            (r2.home_team_id == r.home_team_id or r2.away_team_id == r.home_team_id)
            and r2.kickoff_at_utc > r.kickoff_at_utc
            for r2 in rows
        )
        away_in_later = any(
            (r2.home_team_id == r.away_team_id or r2.away_team_id == r.away_team_id)
            and r2.kickoff_at_utc > r.kickoff_at_utc
            for r2 in rows
        )
        if home_in_later and not away_in_later:
            eliminated.add(r.away_team_id)
        elif away_in_later and not home_in_later:
            eliminated.add(r.home_team_id)

    all_knockout_ids = {r.home_team_id for r in rows} | {r.away_team_id for r in rows}
    return all_knockout_ids - eliminated


@app.get("/ranking/champion-bonus")
def get_champion_bonus_ranking(db: Session = Depends(get_db)):
    alive_ids = _compute_alive_teams(db)

    picks = db.query(ChampionPick.user_id, ChampionPick.team_id).all()
    user_champion = {p.user_id: p.team_id for p in picks}
        ...
        WITH match_pts AS (
            SELECT b.user_id,
                   SUM(CASE
                       ...
                       ELSE 0
                   END) AS pts,
                   SUM(CASE WHEN b.home_score_prediction = m.home_score AND b.away_score_prediction = m.away_score THEN 1 ELSE 0 END) AS exact_scores,
                   SUM(CASE
                       WHEN (b.home_score_prediction > b.away_score_prediction AND m.home_score > m.away_score)
                         OR (b.home_score_prediction < b.away_score_prediction AND m.home_score < m.away_score)
                         OR (b.home_score_prediction = b.away_score_prediction AND m.home_score = m.away_score)
                   ...
                   END) AS correct_results,
                   SUM(CASE
                       WHEN m.home_score > m.away_score AND b.home_score_prediction > b.away_score_prediction AND b.home_score_prediction = m.home_score AND b.away_score_prediction != m.away_score THEN 1
                       WHEN m.away_score > m.home_score AND b.away_score_prediction > b.home_score_prediction AND b.away_score_prediction = m.away_score AND b.home_score_prediction != m.home_score THEN 1
                       ELSE 0
                   END) AS winner_goals,
                   SUM(CASE
                       WHEN m.home_score > m.away_score AND b.home_score_prediction > b.away_score_prediction AND b.away_score_prediction = m.away_score AND b.home_score_prediction != m.home_score THEN 1
                       WHEN m.away_score > m.home_score AND b.away_score_prediction > b.home_score_prediction AND b.home_score_prediction = m.home_score AND b.away_score_prediction != m.away_score THEN 1
                       ELSE 0
                   END) AS loser_goals
            FROM bets b
            JOIN matches m ON m.id = b.match_id
    ...
    rows = db.execute(sql, {"hidden_profile": HIDDEN_FROM_RANKING_PROFILE}).fetchall()

    results = []
    for row in rows:
        champion_alive = user_champion.get(row.user_id) in alive_ids
        total_pts = row.base_pts + (40 if champion_alive else 0)
        results.append(RankingBonusItem(
            user_id=row.user_id,
            user_name=row.user_name,
            total_points=total_pts,
            base_points=row.base_pts,
    ...
    return {
        "ranking": [r.model_dump() for r in results],
        "alive_team_ids": list(alive_ids),
    }
